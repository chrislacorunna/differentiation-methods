...
for (mod, name, nargs) in keys(DiffRules.DEFINED_DIFFRULES)
    ...
    if nargs == 1
        ...
        @eval begin
            ...
            gradient(::typeof($(f_ex_head)), grad, out_size, output, x) = (grad * $df_ex, )
            gradient(mt::Trait.Broadcasted{typeof($f_ex_head)}, grad, out_size, output, x) = (@.(grad * $(df_ex)), )
        end
    elseif nargs == 2
        ...
        @eval begin
            ...
            gradient(::typeof($f_ex_head), grad, out_size, output, x, y) =
                (grad * $(df_ex[1]), grad * $(df_ex[2]))
            gradient(::Trait.Broadcasted{typeof($f_ex_head)}, grad, out_size, output, x, y) =
                (@.(grad * ($(df_ex[1]))), @.(grad * $(df_ex[2])))
        end
    else
...
gradient(::typeof(Base.abs), grad, out_size, output, x) = (grad * abs_gradient(x), )
gradient(::Trait.Broadcasted{typeof(Base.abs)}, grad, out_size, output, x) = (@.(grad * abs_gradient(x)), )
