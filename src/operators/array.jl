...
function gradient(::typeof(Base.getindex), grad, out_size, output, x::AbstractArray, inds...)
    grad_output = fill!(similar(x), 0)
    setindex!(grad_output, grad, inds...)
    (grad_output, )
end

gradient(::typeof(Base.getindex), grad, out_size, output, x::Number, ind::Int) = (grad, )

Base.selectdim(x::AbstractNode, d, i) = register(Base.selectdim, x, d, i)

function gradient(::typeof(Base.selectdim), grad, out_size, output, x::AbstractArray, d, i)
    grad_output = fill!(similar(x), 0)
...
function gradient(::typeof(Base.view), grad, out_size, output, x::AbstractArray, inds...)
    grad_output = fill!(similar(x), 0)
