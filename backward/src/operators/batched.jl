...
function gradient(::typeof(Batched.gemm), grad, out_size, output, A::AbstractArray{T, 3}, B::AbstractArray{T, 3}) where T
    Batched.gemm(grad, transpose(B)), Batched.gemm(transpose(A), grad) # possibbly have to insert into a tuple
end

Batched.tr(A::AbstractNode) = register(Batched.tr, A)

function gradient(::typeof(Batched.tr), grad, out_size, output, A::AbstractArray{T, 3}) where T
